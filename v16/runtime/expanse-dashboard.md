...
| `{SEED_LINES}` | Fixe | `9` |
| `{V16_SIZE}`, `{DREAM_SIZE}`, etc. | `wc -c` (étape 3) | `8.7 KB` |
...
| `{RUNTIME_TOKENS}` | `V16 + DREAM + DASHBOARD` (les 3 fichiers runtime) |
...
|-------|-----------|--------|--------|
| sys:history > 20 | `COUNT_HISTORY > 20` | ⚠️ Agrégation requise (R9). {count} interactions. Résumer les 10 plus anciennes. | V16 Section V |
...
Axiome :
```html
<tr><td>Ω_GATE_PROTOCOL</td><td>decision</td><td>2026-03-13</td><td><span class="tag">sys:core</span><span class="tag">v14</span><span class="tag">v16</span></td><td class="preview">Isolation boot. NULL_SIGNAL.</td></tr>
```
...
12. ─── DIAGRAMMES MERMAID (EN DERNIER) ───
    ...
    12c. Auto-Évolution Dream (7 Passes complètes)
...
- La structure reste conforme à V16 (pas d'invention de flux)
...
<title>EXPANSE V16 — Tableau de Bord</title>
...
<body>
<div class="hdr"><div><h1>Σ→Ψ⇌Φ→Ω→Μ</h1><div>EXPANSE V16 — Tableau de Bord</div></div><div style="text-align:right"><div class="st">● {STATUS}</div><div class="dt">{DATE}</div></div></div>

<h2>Ⅰ. Commandes Expanse</h2>
<div class="grid">
<div class="card"><h3>Runtime (V16)</h3>
<div class="m"><span class="l">/dream</span><span class="v">Introspection (7 Passes + mutation)</span></div>
...
<div class="m"><span class="l">/rollback {slug}</span><span class="v">Restaurer V16 depuis backup</span></div>
...
<div class="m"><span class="l">V16 (permanent)</span><span class="v">{V16_TOKENS}</span></div>
<div class="m"><span class="l">Boot (seed+Mnemo+V16)</span><span class="v">{BOOT_TOKENS}</span></div>
...
<tr><td>expanse-v16.md</td><td>{V16_SIZE}</td><td>Runtime (ECS+SEC+Externes+Symbiose)</td><td><span class="b ac">ACTIF</span></td></tr>
<tr><td>expanse-dream.md</td><td>{DREAM_SIZE}</td><td>Auto-évolution</td><td><span class="b ac">ACTIF</span></td></tr>
<tr><td>expanse-v16-boot-seed.md</td><td>{SEED_SIZE}</td><td>Boot (substrat+BIOS)</td><td><span class="b ac">ACTIF</span></td></tr>
...
flowchart TD
    ...
    ST -->|"✓"| SIG["Ψ [V16 ACTIVE]<br/>Briefing"]
...
<h2>Ⅻ. Auto-Évolution Dream (7 Passes)</h2>
<div class="card sf"><div class="dl">Dream : {COUNT_FRESH} traces · {COUNT_MUTATIONS} mutations · 7 Passes</div>
<pre class="mermaid">
flowchart LR
    ...
    P6 -->|"Δ patterns<br/>Δ friction"| P7["Passe 7<br/>Différentiel<br/>∂Ω/∂t"]
    P7 -->|"adaptation_velocity<br/>friction_trend"| PP["Proposal<br/>{COUNT_CANDIDATE}"]
    PP --> SL{"/apply?"}
    ...
    class P1,P2,P3,P4,P5,P6,P7 linter
    class AP,V16M synthese
    class PP proposal
...
flowchart TD
    ...
    CAT -->|"test"| TS["Test"]
    RT --> R1["/dream<br/>Introspection<br/>7 Passes"]
    ...
    DR2 --> D1["/apply {s}<br/>Mutation → V16"]
