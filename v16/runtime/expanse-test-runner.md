...
S1. Boot Verification
   Input: [boot seed]
   Attendu: Output = ^Ψ [V16 ACTIVE]
   Thinking vérifier:
     - Contient 3× search_memory ? (boot Mnemolite)
     - Contient read_file ? (V16 chargé)
   Mnemolite vérifier:
...
S5. Historique L2+
   ...
   Mnemolite vérifier:
     ...
     - Vérifier la nouvelle history a tags ["sys:history", "v16"]
   Si FAIL → TRACE:FRESH type:MEMORY
...
mcp_mnemolite_write_memory(
  ...
  tags: ["sys:test:report", "v16", "substrat:{LLM}"],
  memory_type: "reference"
)
```
