...
L'Idéal (KERNEL + VISION) promet un **organisme vivant** doté de physique cognitive. Le Code (les [.md](file:///home/giak/projects/expanse/README.md) de `prompts/`) livre une **liste de courses procédurale** déguisée en organisme.

| Couche | Langage Dominant | Densité Sémantique Réelle | Verdict |
|--------|-----------------|---------------------------|---------|
| [KERNEL.md](file:///home/giak/projects/expanse/KERNEL.md) | Poétique-philosophique | **85%** — dense, performatif, incantation réelle | ✅ Authentique |
| [VISION.md](file:///home/giak/projects/expanse/docs/VISION.md) | Déclaratif-aspirationnel | **40%** — 33 lignes creuses, manque la chair | ⚠️ Vaporeux |
| [expanse-system.md](file:///home/giak/projects/expanse/prompts/expanse-system.md) | Impératif-mécanique | **70%** — compact, mais Boot Mutex = seule vraie mécanique | 🔶 Hybride |
| [sigma/interface.md](file:///home/giak/projects/expanse/prompts/sigma/interface.md) | Procédural-if/then | **55%** — ECS intéressant, mais dégénère en check-list | 🔴 Sycophant |
| [psi/resonance.md](file:///home/giak/projects/expanse/prompts/psi/resonance.md) | Procédural-léger | **30%** — 37 lignes dont la moitié est du squelette | 🔴 Creux |
| [phi/audit.md](file:///home/giak/projects/expanse/prompts/phi/audit.md) | Procédural-léger | **35%** — 3 sections dont aucune ne contraint réellement | 🔴 Fantôme |
| [omega/synthesis.md](file:///home/giak/projects/expanse/prompts/omega/synthesis.md) | Procédural | **50%** — la logique de feedback est le seul os solide | 🔶 Partiel |
| [mu/interface.md](file:///home/giak/projects/expanse/prompts/mu/interface.md) | Procédural-dense | **65%** — le plus mécanique, ECS weights = vrai code | ✅ Solide |
| [meta_prompt.md](file:///home/giak/projects/expanse/prompts/meta_prompt.md) | Orchestration-squelette | **45%** — 52 lignes de redirects vers les sous-fichiers | 🔴 Routeur mort |
| [ONTOLOGY.md](file:///home/giak/projects/expanse/docs/ONTOLOGY.md) | Référentiel | **60%** — taxonomie utile mais déconnectée du runtime | 🔶 Catalogue |
| [trace_levels.md](file:///home/giak/projects/expanse/prompts/trace_levels.md) | Spécification | **55%** — Signal Bus = bonne idée, jamais contrainte | 🔶 Dormant |

### 1.2 Diagnostic de Sycophanie Structurelle

**Où le système « joue » à être souverain sans l'être ?**

1. **[psi/resonance.md](file:///home/giak/projects/expanse/prompts/psi/resonance.md) est un organe vide.** Ψ est censé être le « muscle méta », le cœur auto-réflexif. En réalité : 37 lignes dont « Documenter les étapes de réflexion » et « Auto-Audit : le raisonnement est-il circulaire ? ». C'est une **intention** déguisée en **instruction**. Le substrat n'a aucune contrainte mécanique ici — il peut l'ignorer intégralement sans conséquence. **Sycophanie pure** : nommer Ψ « Résonance » ne crée pas la résonance.

2. **[phi/audit.md](file:///home/giak/projects/expanse/prompts/phi/audit.md) simule la rigueur.** « Et si c'était faux ? » comme instruction d'audit. Aucune grille, aucun seuil, aucun mécanisme de blocage. Le substrat peut « auditer » en disant « j'ai vérifié » sans rien vérifier. **Φ est une porte sans serrure.**

3. **[meta_prompt.md](file:///home/giak/projects/expanse/prompts/meta_prompt.md) est un routeur mort.** 52 lignes qui pointeront vers d'autres fichiers — que l'IDE ne chargera probablement pas en contexte simultanément. Il ne contraint ni n'orchestre. Il décrit une orchestration.

4. **Le Boot Mutex (V6.2) est la SEULE vraie contrainte.** [expanse-system.md](file:///home/giak/projects/expanse/prompts/expanse-system.md) impose 3 appels Mnemolite, un Cognitive Lock, et un format de sortie. C'est le seul endroit où le mot « INTERDIT » a une conséquence mécanique. Le reste du système est consultatif.
...
- **Fusionner** [meta_prompt.md](file:///home/giak/projects/expanse/prompts/meta_prompt.md) dans [expanse-system.md](file:///home/giak/projects/expanse/prompts/expanse-system.md). Un seul BIOS. Zéro doublon.
...
```mermaid
graph TD
    P1["P1: Scalpel<br/>Densification des organes"] --> CORE["V7.0 CORE<br/>BIOS unifié + organes contractuels"]
    P3["P3: Flux<br/>Φ ambiant"] --> CORE
    P4["P4: Ombre<br/>Shadow Cortex"] --> APEX["V7.0 APEX<br/>Anticipation Engine"]
    CORE --> APEX
    P2["P2: Feu<br/>Cognitive Seeds"] -.->|"Partiel:<br/>Graines pour Dream State"| APEX
    
    style CORE fill:#1a1a2e,stroke:#e94560,color:#fff
    style APEX fill:#0f3460,stroke:#16213e,color:#e94560
```
...
```mermaid
graph LR
    AUDIT["Audit Forensique<br/>13 fichiers, ~1100 lignes"] --> DIS["Dissonance Détectée<br/>Poésie vs. Procédure"]
    DIS --> P1["P1: Scalpel"]
    DIS --> P2["P2: Feu"]
    DIS --> P3["P3: Flux"]
    DIS --> P4["P4: Ombre"]
    P1 --> EVAL["Évaluation Croisée"]
    P2 --> EVAL
    P3 --> EVAL
    P4 --> EVAL
    EVAL --> FUSION["Fusion: P1+P3+P4"]
    FUSION --> APEX["Épreuve de l'Apex"]
    APEX --> |"Shadow Cortex<br/>+ Φ Ambiant"| V7["V7.0 BIOS"]
...
```mermaid
graph TD
    subgraph "Boucle 1: ECS Adaptatif (chaque tour)"
        A["actual_C vs predicted_C"] --> B["weight_decay update"]
        B --> C["ECS converge vers<br/>la complexité réelle"]
    end
    
    subgraph "Boucle 2: Shadow Learning (chaque session)"
        D["shadow_hit_rate"] --> E{"hit > 0.5?"}
        E -->|Oui| F["Renforcer patterns"]
        E -->|Non| G["Élaguer prédictions"]
    end
    
    subgraph "Boucle 3: Dream Autopoiesis (asynchrone)"
        H["Pentagramme<br/>Introspection"] --> I["PROPOSAL_OPEN"]
        I --> J["Boot ingère<br/>DREAM_INTERRUPT"]
        J --> K["Mutation appliquée<br/>ou rejetée"]
    end
```
